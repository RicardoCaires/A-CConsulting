# Wenn ein Schaden eintritt — Vorgaben Ricardo, 11.09.2026

Aus dem Auftrag vom 11.09.2026, den Abschnitt nach der Referenzgrafik neu zu
bauen (`bilder-quelle/versicherungen_schadenfall_referenz.png`).

**Titel, Lead und Linkbeschriftung sind unverändert** aus
`schritt4_fassung2_de.md`. Neu sind nur die folgenden Zeilen, die Ricardo im
Auftrag wörtlich vorgegeben hat.

## Kategoriezeile

Schadenfall

## Die drei Schritte

Fall aufnehmen

Versicherung informieren

Abwicklung begleiten

## Die Partnerkarte

Für die Schadenmeldung auf Ihre Gesellschaft klicken.

Allianz — Schaden melden

AXA — Schaden melden

die Mobiliar — Schaden melden

Generali — Schaden melden

GVB — Schaden melden

Helvetia — Schaden melden

Smile — Schaden melden

Vaudoise — Schaden melden

Zurich — Schaden melden

## Der Knopf

Herunterladen

## Anmerkungen

**Die Referenzgrafik zeigt einen anderen Wortlaut** als der Auftrag: Titel
„Wir begleiten Sie durch den Schadenfall.", Lead „Melden Sie sich direkt bei
uns …", Link „Jetzt Schadenfall melden" und drei längere Schrittbeschriftungen.
Der Auftragstext nennt ausdrücklich den bestehenden Wortlaut; der ist
freigegeben und steht deshalb weiterhin auf der Seite.

Die Grafik trägt ausserdem drei Zusätze („Starke Partner. Für Ihre
Sicherheit.", „Gemeinsam Lösungen finden.", „Persönlich. Unabhängig. An Ihrer
Seite."). Sie sind **nicht** übernommen: Ricardo hat am 10.09.2026 zwei
Zusätze von dieser Seite streichen lassen.

**Für die Partnerkarte war kein Zusatztext vorgegeben.** Der Auftrag nennt
„kurzer Zusatztext", ohne ihn zu schreiben. Es wird keiner erfunden; die Karte
trägt Titel und Logoraster.

**Das Logoraster zeigt neun Gesellschaften und weicht vom Raster im
Modellabschnitt ab** (dort Swiss Life statt GVB und Smile). Auf derselben
Seite stehen damit zwei verschiedene Aufzählungen. Ricardo ist darauf
hingewiesen.

## Änderungen vom 11.09.2026

Auf Ricardos Anweisung:

- **Der Grund ist weiss**, das Hintergrundmuster ist heraus.
- **„Wir begleiten Sie persönlich" fluchtet mit den Logokacheln** — das Symbol
  steht darüber statt daneben.
- **Jedes Logo ist ein Link** auf die Schadenmeldung seiner Gesellschaft. Die
  Adressen stammen von der Übersicht, die Ricardo genannt hat
  (`providereconcepts.ch/schadenfall`). Dafür ist das gelieferte Raster in
  seine neun Kacheln geschnitten — mechanisch, entlang der Kartenkanten,
  nichts umgezeichnet.
- **Statt des Vermerks „folgt" steht ein grüner Knopf „Herunterladen".** Das
  PDF fehlt noch; Ricardo legt es später ab. Bis dahin führt der Knopf ins
  Leere. Der offene Punkt steht in CLAUDE.md.

Noch am selben Tag nachgeschärft, ebenfalls auf seine Anweisung:

- **Das Schildsymbol über der Partnerkarte ist entfallen.**
- **„Wir begleiten Sie persönlich" ist ersetzt** durch „Für die Schadenmeldung
  auf Ihre Gesellschaft klicken." — aus einer Überschrift ist eine Anleitung
  geworden. Ricardo hat den Wortlaut offengelassen („oder etwas in dieser
  Art"); dies ist die knappste Fassung in der Sie-Form.
- Der Hinweis ist **grün unterstrichen**, wie die Kontaktangaben im
  Abschluss der Startseite.
- Die **Bewegung beim Darüberfahren ist verstärkt**: Die Kachel hebt sich
  deutlicher und bekommt einen weichen Schatten.
